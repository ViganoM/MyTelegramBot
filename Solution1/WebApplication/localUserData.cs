...
namespace myTelegramBot {
    ...
    static class localUsersData {
        ...
        public static string SaveFilepath {
            get {
                string path = Settings.Default.settingsFoldername;
                if ( !Directory.Exists(path) )
                    Directory.CreateDirectory(path);
                return path + Settings.Default.settingFilename;
            }
        }
        public static string LogFilepath {
            get {
                if ( !Directory.Exists(Settings.Default.settingsFoldername) )
                    Directory.CreateDirectory(Settings.Default.settingsFoldername);
                return Settings.Default.settingsFoldername + Settings.Default.settingFilename + "_LOG.txt";
            }
        }
        ...
        static void SecondSave() {
            ...
            Program.WriteException( new string[] { "-------", now, "Data not saved" });
        }
        ...
        static bool SecondLoad() {
            FileInfo[] list = new DirectoryInfo(Settings.Default.settingsFoldername).GetFiles().Where(x => x.Name.StartsWith(Settings.Default.settingFilename)).Where(x => !x.Name.Contains("_LOG")).OrderByDescending(x => x.CreationTime).ToArray();
            foreach ( FileInfo file in list )
                try {
                    ...
                    //TODO if graph is null throw ex
                    ...
                    return true;
                        ...
                        Program.WriteException(serializationEx.ToString().Split(new string[] { "\n" }, StringSplitOptions.None).ToArray());
                } catch ( UnauthorizedAccessException unautorizes ) {
                       Program.WriteException(unautorizes.ToString().Split(new string[] { "\n" }, StringSplitOptions.None).ToArray());
                } catch ( IOException ioexception ) {
                       Program.WriteException(ioexception.ToString().Split(new string[] { "\n" }, StringSplitOptions.None).ToArray());
                }

               Program.WriteException(new Exception("Settings could not be restored from any file in path:\n" + Settings.Default.settingsFoldername).ToString().Split(new string[] { "\n" }, StringSplitOptions.None).ToArray());
            return false;
        }

        public static void DeleteOld() {
            FileInfo[] files = new DirectoryInfo(Settings.Default.settingsFoldername).GetFiles("*", SearchOption.AllDirectories).OrderBy(x => x.CreationTime).ToArray();
